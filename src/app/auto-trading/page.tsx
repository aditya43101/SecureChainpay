...
export default function AutoTradingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6 backdrop-blur-sm">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 flex items-center gap-4">
              <span>All-Time Mode: <strong className={settings.allTimeMode ? 'text-cyan-400' : 'text-slate-500'}>{settings.allTimeMode ? 'ACTIVE' : 'OFF'}</strong></span>
              <span>•</span>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition-all flex items-center gap-2"
          >
          ...
          {!isEnabled ? (
            <button
              ...
              className="px-5 py-2.5 rounded-xl text-xs font-black bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-2"
            >
      ...
      {settings.allTimeMode && (
        <div className="bg-gradient-to-r from-cyan-950/60 via-slate-900 to-indigo-950/60 border border-cyan-500/30 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping" />
            <div>
              <div className="text-xs font-bold text-cyan-300 uppercase tracking-wider">All-Time Monitoring Active</div>
              <div className="text-xs text-slate-400">
                Continuously analyzing market data for BTCUSDT & ETHUSDT. Will execute only if all 8 safety gates pass.
              </div>
            </div>
          </div>
          <span className="text-xs font-mono text-cyan-400/80 bg-cyan-950 border border-cyan-800 rounded-md px-2.5 py-1">
            Interval: 15s
          </span>
        </div>
      )}
      ...
      {cycleLogs.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2">
          ...
          <div className="bg-slate-950 rounded-lg p-3 text-[11px] font-mono text-slate-300 max-h-32 overflow-y-auto space-y-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
          ...
          <div className="text-[11px] text-slate-400 mt-1">Configured Position Size Risk</div>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
          ...
          <div className="text-[11px] text-slate-400 mt-1">
            Today P&L: ${(dailyState.realizedPnL || 0).toFixed(2)}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {account.positions.length === 0 ? (
              <div className="bg-slate-950/60 rounded-xl p-8 text-center text-slate-400 text-xs border border-slate-800/60">
                No active auto-traded positions. Monitoring market for valid setups...
              </div>
            ) : (
              <div className="space-y-3">
                {account.positions.map((pos: any) => (
                  <div key={pos.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {account.orders.length === 0 ? (
              <div className="bg-slate-950/60 rounded-xl p-6 text-center text-slate-400 text-xs border border-slate-800/60">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {events.length === 0 ? (
              ...
              <div className="space-y-2.5 max-h-[500px] overflow-y-auto pr-1">
                {events.map((ev: any) => (
                  <div key={ev.id} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs space-y-1">
                    <div className="flex items-center justify-between">
