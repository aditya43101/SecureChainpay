...
export async function GET(
  ...
  try {
    ...
    if (clean.endsWith('USDT')) {
      ...
      formattedSymbol = clean.replace(/USD$/, 'USDT');
    } else {
      formattedSymbol = `${clean}USDT`;
    }
    
    // Normalize timeframe
    const validTimeframes = ['1m', '5m', '15m', '1h', '4h', '1d'];
    const selectedTimeframe = validTimeframes.includes(timeframe.toLowerCase()) ? timeframe.toLowerCase() : '1h';
