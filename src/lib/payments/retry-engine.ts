...
export class PaymentRetryEngine {
  ...
  static async handleExecutionOutcome(intentId: string, outcome: { type: 'SUCCESS' | 'ERROR' | 'UNKNOWN', code?: string, executionId: string }) {
    try {
      ...
      const maxAttempts = 3;
      const currentAttempts = intent?.executions?.length || 1;
      ...
      console.warn(`[PaymentRetryEngine] DB error during handleExecutionOutcome for ${intentId}:`, err?.message || err);
      if (outcome.type === 'SUCCESS') {
        return await PaymentContinuityService.transitionState(intentId, 'CONFIRMED', 'Transaction confirmed');
      }
      return await PaymentContinuityService.transitionState(intentId, 'BROADCAST_UNKNOWN', 'Unknown outcome');
    }
  }
}
